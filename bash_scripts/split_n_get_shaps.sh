...
if [ "$method" == "n" ]
    then
        ...
        sbatch bash_scripts/get_plate_shaps.sh data/HRCE-1/"$data" "HRCE-1"
        sbatch bash_scripts/get_plate_shaps.sh data/HRCE-2/"$data" "HRCE-2"
        sbatch bash_scripts/get_plate_shaps.sh data/VERO-1/"$data" "VERO-1"
        sbatch bash_scripts/get_plate_shaps.sh data/VERO-2/"$data" "VERO-2"

        # Calculate disease condition shap values
        sbatch bash_scripts/get_disease_condition_shaps.sh data/HRCE-1/"$data" "HRCE-1"
        sbatch bash_scripts/get_disease_condition_shaps.sh data/HRCE-2/"$data" "HRCE-2"
        sbatch bash_scripts/get_disease_condition_shaps.sh data/VERO-1/"$data" "VERO-1"
        sbatch bash_scripts/get_disease_condition_shaps.sh data/VERO-2/"$data" "VERO-2"
fi